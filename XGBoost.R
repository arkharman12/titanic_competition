...
submission <- read.csv("test.csv", stringsAsFactors = F)
...
feature_eng <- function(train_df, test_df) {
  ...
  # Create a new familyID2 variable that reduces the factor level of falilyID 
...
# Call the engineering function
data <- feature_eng(train, test)

# Create a dataframe containing only columns that interest us and converting the data into numerics in order to use xgboost
...
# Use the cross validation to estimate our error rate:
...
dim(proportion)
# Apply the best cut-off on the train set prediction for score checking
...
# Create the submitting file
KaggleSubmission6 <- data.frame(PassengerId = data[892:1309,c("PassengerId")], Survived = predict_xgboost_test)
write.csv(KaggleSubmission6, file = "KaggleSubmission6.csv", row.names = FALSE)
